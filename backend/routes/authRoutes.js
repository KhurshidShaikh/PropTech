  ...
  const {username,email,password}=req.body;
   ...
   if(existinguser){
     return res.json({message:"user already exists"})
        ...
        const otp=otpgenerator.generate(4,{specialChars:false,upperCaseAlphabets:false,lowerCaseAlphabets:false})
        console.log(otp);
...
router.post("/verifyotp/:id",async(req,res)=>{
  ...
  if(otp===user.otp){
    ...
    return res.json({message:"Internal server Error . Please try again later"})
...
try {
  ...
  const user= await userModel.findOneAndUpdate({_id:id},{password:newhashpassword})
  if(!user){

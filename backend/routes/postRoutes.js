...
try {
    ...
    const updatedpost= await postModel.findByIdAndUpdate(id,{basicInfo},{new:true})
  
    if (postDetails) {
        const postDetailDoc = await postDetailModel.findByIdAndUpdate(post.postDetail, postDetails, { new: true });
        if (!postDetailDoc) {
...
try {
    ...
    if(savedPost){
        ...
        user.savedPost=user.savedPost.filter(post=>!post.equals(savedPost._id))
        await user.save()

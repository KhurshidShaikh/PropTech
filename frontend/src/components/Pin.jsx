      ...
      if(posts){
        const coords = await Promise.all(
          posts.map(async (post) => {
            console.log('Fetching coordinates for address:', post.basicInfo.address);
        ...
        console.log('Fetching coordinates for address:', property.basicInfo.address);
        ...
        <Marker key={index} position={post.coordinates}>
          <Popup>
            <div className="popup-card">
              <img src={post.basicInfo.images[0]} alt="" style={{ maxWidth: '100%', height: 'auto' }} />
